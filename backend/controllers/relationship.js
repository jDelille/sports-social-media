  ...
  jwt.verify(token, process.env.SECRET_KEY, (err, userInfo) => {
    ...
    db.query(followStatusQuery, [currentUserId, userId], (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Error fetching follow status" });
      }
  ...
  jwt.verify(token, process.env.SECRET_KEY, (err, userInfo) => {
    ...
    db.query(followerCountQuery, [userId], (err, followerData) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Error fetching follower count" });
      }
      ...
      db.query(followingCountQuery, [userId], (err, followingData) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Error fetching following count" });
        }
        ...
        db.query(followersIdsQuery, [userId], (err, followers) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ message: "Error fetching followers IDs" });
          }
          ...
          db.query(followingIdsQuery, [userId], (err, following) => {
            if (err) {
              ...
              return res
                .status(500)
                .json({ message: "Error fetching following IDs" });
            }

            // Map results to arrays of IDs
            const followerIdsArray = followers.map(
              (follower) => follower.follower_id
            );
            const followingIdsArray = following.map(
...
export const getFollowingUsers = (req, res) => {
  ...
  db.query(followingIdsQuery, [userId], (err, following) => {
    ...
    db.query(
      followedUsersQuery,
      [followedUserIds, offset, pageSize],
      (err, followedUsers) => {
        if (err) {
          console.error("Error fetching followed user details:", err);
          return res
            .status(500)
            .json({ message: "Error fetching followed user details" });
        }
...
export const getFollowerUsers = (req, res) => {
  ...
  db.query(followerIdsQuery, [userId], (err, followers) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error fetching follower user IDs" });
    }
    ...
    db.query(
      followersQuery,
      [followerUserIds, offset, pageSize],
      (err, followerUsers) => {
        if (err) {
          console.error("Error fetching follower user details:", err);
          return res
            .status(500)
            .json({ message: "Error fetching follower user details" });
        }
  ...
  db.query(followerCountQuery, [userId], (err, followerData) => {
    ...
    db.query(followingCountQuery, [userId], (err, followingData) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error fetching following count" });
