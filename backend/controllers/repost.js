...
export const addRepost = (req, res) => {
  ...
  jwt.verify(token, process.env.SECRET_KEY, (err, userInfo) => {
      ...
      if (req.body.type === 'post' || req.body.type === "repost") {
          ...
          q = "INSERT INTO defaultdb.comments (`comment_id`, `reposter_id`, `reposter_username`, `created_at`) VALUES (?)";
      }
  ...
  export const removeRepost = (req, res) => {
    ...
    jwt.verify(token, process.env.SECRET_KEY, (err, userInfo) => {
        ...
        if (req.body.type === 'post' || req.body.type === "repost") {
            ...
            q = "DELETE FROM defaultdb.comments WHERE comment_id = ? AND reposter_id = ?";
        }
...
export const isPostReposted = (req, res) => {
  ...
  jwt.verify(token, process.env.SECRET_KEY, (err, userInfo) => {
      ...
      if (req.query.type === 'post' || req.query.type === "repost") {
          ...
          q = "SELECT * FROM defaultdb.comments WHERE comment_id = ? AND reposter_id = ?";
      }
