...
export const getLikes = (req, res) => {
  ...
  if (req.query.type === "post" || req.query.type === "repost") {
    ...
    q = "SELECT user_id FROM defaultdb.likes WHERE comment_id = ?";
    values = [req.query.postId];
  } else {
...
export const addLike = (req, res) => {
  ...
  jwt.verify(token, process.env.SECRET_KEY, (err, userInfo) => {
    ...
    if (req.body.type === "post" || req.body.type === "repost") {
      ...
      q = "INSERT INTO defaultdb.likes (`user_id`, `comment_id`) VALUES (?, ?)";
      values = [userInfo.id, req.body.postId];
    } else {
      return res.status(400).json("Invalid type");
    }
    ...
    db.query(q, values, (err, data) => {
      if (err) return res.status(500).json(err);
      return res.status(200).json("Item has been liked.");
    });
  });
};
...
export const removeLike = (req, res) => {
  ...
  jwt.verify(token, process.env.SECRET_KEY, (err, userInfo) => {
    ...
    if (req.body.type === "post" || req.body.type === "repost" ) {
      ...
      q = "DELETE FROM defaultdb.likes WHERE user_id = ? AND comment_id = ?";
      values = [userInfo.id, req.body.postId];
    } else {
      return res.status(400).json("Invalid type");
    }
